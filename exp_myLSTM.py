...
class experiment:
    def __init__(self,model,trainset,testset,output_labels,params,exp_name,modelPath,skiptuning,batch_size=1,gen_output=False):
        self.model = model
        self.params = params
...
def run(w2v_option, lstm_hid_dim, nn_hid_dim, pos_emb_dim, common_sense_emb_dim, cnn_filter_sizes, cnn_filter_num, bigramstats_dim, granularity, lr, weight_decay, step_size, gamma, max_epoch, expname, skiptuning, skiptraining, gen_output, mode, dropout, timeline_kb, bilstm, debug, batch_size, sd):
    seed_everything(sd)
    ...
    # obsolete
    ...
    elif mode == 17: # mode=17 CNN with dropout for common sense embeddings
        ratio = 0.3
        emb_size = 200
        layer = 1
        print("---------")
        ...
        print("ratio=%s,emb_size=%d,layer=%d,lemma_emb_dim=%d" % (str(ratio), emb_size, layer, params['lemma_emb_dim']))
        bigramGetter = bigramGetter_fromNN(emb_path, mdl_path, ratio, layer, emb_size, splitter=',')
        model = CNN_bigramStats(params, emb_cache, bigramGetter, position2ix).cuda()
    else:
